...
SPARK_MASTER_CONTAINER = "laba5-spark-master"
...
def build_env() -> dict[str, str]:
    ...
    env.setdefault("POSTGRES_DB", dotenv.get("POSTGRES_DB", "laba5"))
    env.setdefault("POSTGRES_USER", dotenv.get("POSTGRES_USER", "laba5"))
    env.setdefault("POSTGRES_PASSWORD", dotenv.get("POSTGRES_PASSWORD", "laba5"))
    return env
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Laba5 air, weather and NMU Spark ML pipeline.")
    ...
    parser.add_argument(
        ...
        choices=range(1, 7),
        default=1,
        help="Start from a pipeline step: 1..6.",
    )
...
def main() -> None:
    ...
    if start_step <= 1:
        print("\n[1/6] Starting infrastructure")
        if args.restart_services:
            restart_infrastructure()
        else:
            start_infrastructure()
    else:
        print("\n[1/6] Skipping infrastructure")

    if start_step <= 2:
        print("\n[2/6] Downloading raw air, weather, site and NMU data")
        download_raw_data(args, env)
    else:
        print("\n[2/6] Skipping raw download")

    if start_step <= 3:
        print("\n[3/6] Normalizing source data to staging with Spark")
        normalize_sources_with_spark()
    else:
        print("\n[3/6] Skipping source normalization")

    if start_step <= 4:
        print("\n[4/6] Building air-weather marts with Spark")
        build_marts_with_spark()
    else:
        print("\n[4/6] Skipping Spark marts")

    if start_step <= 5:
        print("\n[5/6] Training Spark ML models (regression, classification, clustering)")
        train_ml_models_with_spark(args)
    else:
        print("\n[5/6] Skipping Spark ML")

    if start_step <= 6:
        print("\n[6/6] Loading staging tables, marts and ML results into Postgres")
        load_to_postgres(env)
    else:
        print("\n[6/6] Skipping Postgres load")
