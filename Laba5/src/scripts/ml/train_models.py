"""Spark ML stage for Laba5.

Builds a feature table from three sources of different nature:

* structured: cleaned air measurements + cleaned Open-Meteo weather (CSV);
* unstructured: NMU announcement text expanded to an hourly grid, turned into TF-IDF features.

On top of that table it trains three families of models and exports Superset-friendly CSV marts:

* regression   -> predict AQI from weather / time / NMU features;
* classification -> predict the pollution-peak flag (is_pollution_peak);
* clustering    -> KMeans over air+weather conditions.
"""
...
# Numeric predictors shared by the supervised models.
SUP_NUMERIC = [
    "hour",
    "month",
    "weather_temperature",
...
# Numeric features describing the "condition" for clustering.
CLUSTER_NUMERIC = [
    "aqi",
    "pm25",
    "pm10",
