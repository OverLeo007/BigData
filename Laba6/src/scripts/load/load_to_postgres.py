...
POSTGRES_DB = os.getenv("POSTGRES_DB", "laba6")
POSTGRES_USER = os.getenv("POSTGRES_USER", "laba6")
POSTGRES_PASSWORD = os.getenv("POSTGRES_PASSWORD", "laba6")
...
# (scripts/stream/stream_processing.py) напрямую через JDBC. Создаются отдельной стадией пайплайна
# (--stream-views-only) ПОСЛЕ обработки потока, т.к. таблицы ещё нет на этапе обычного load.
STREAM_AGG_TABLE = "mart_stream_aggregates"
STREAM_VIEW_NAMES = ["v_stream_aggregates", "v_stream_site_summary"]
...
def create_stream_views(engine) -> None:
    sql = """
    create or replace view v_stream_aggregates as
    select
        window_start,
        window_end,
        site_id,
        ...
        peak_count,
        sample_count
    from mart_stream_aggregates;

    create or replace view v_stream_site_summary as
    select
        site_id,
        site_name,
        count(*) as window_count,
        sum(sample_count) as total_samples,
        round(avg(avg_aqi)::numeric, 1) as avg_aqi,
        max(max_aqi) as max_aqi,
        round(avg(avg_pm25)::numeric, 1) as avg_pm25,
        sum(peak_count) as peak_count
    from mart_stream_aggregates
...
def main() -> None:
    ...
    parser.add_argument(
        "--stream-views-only",
        action="store_true",
        help="Создать только потоковые витрины (v_stream_*) над mart_stream_aggregates и выйти.",
    )
    ...
    if args.stream_views_only:
        engine = create_engine(postgres_url())
        create_stream_views(engine)
        print("Stream views created: " + ", ".join(STREAM_VIEW_NAMES))
        return
